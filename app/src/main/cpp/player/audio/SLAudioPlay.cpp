...
void SLAudioPlay::Close()
{
    ...
    XLOGE("SLAudioPlay::iplayer SL_PLAYSTATE_STOPPED");
    //清理播放队列
    if(bf && (*bf))
    {
        (*bf)->Clear(bf);
    }
    XLOGE("SLAudioPlay::bf Clear");
    if(pcmQue && (*pcmQue))
    {
        (*pcmQue)->Clear(pcmQue);
    }
    XLOGE("SLAudioPlay::pcmQue Clear");
    //销毁player对象
    if(player && (*player))
    {
    ...
    XLOGE("SLAudioPlay::player Destroy");
    ...
    XLOGE("SLAudioPlay::engineSL Destroy");
    mux.unlock();
}
