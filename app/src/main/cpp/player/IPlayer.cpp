...
void IPlayer::receiveVideoFrame(int type, int camType, int width, int height, int fps, int64_t pts,
...
void IPlayer::receiveAudioFrame(int type, int channels, int sps, int bps, int64_t pts,
                                unsigned char *dataArray, long size)
{
    if (netAudio)
    {
...
void IPlayer::capture(char *path, int type)
{
    mux.lock();
    /*if (vdecode)
    {
        vdecode->Capture(path,type);
    }*/
    if (videoView)
    {
        videoView->Capture(path,type);
    }
    mux.unlock();
}

void IPlayer::setSkipFrame(int skipNum) {
    mux.lock();
    if (netVideo) {
        netVideo->setSkipFrame(skipNum);
    }
    mux.unlock();
}
