...
bool FFDecode::SendPacket(XData pkt)
{
    ...
    XLOGT("FFDecode pts %lld ,size %d : ", pkt.pts,pkt.size);

    if (pkt.type == UCHAR_TYPE) {
        if (!packet) {
        ...
        packet->pts = pkt.pts;
    } else if (pkt.type == AVPACKET_TYPE) {
        /*
        if (!packet) {
            packet = av_packet_alloc();
        }
        av_packet_ref(packet,(AVPacket *)pkt.data);
         */
        packet =  (AVPacket *)pkt.data;
    } else {
        mux.unlock();
        return false;
    }
    if (pkt.frameType == FRAME_I) {

    }
    //int re = avcodec_send_packet(codec, (AVPacket *)pkt.data);
    //XLOGT("avcodec_send_packet");
    int re = avcodec_send_packet(codec,packet);
    //XLOGT("avcodec_send_packet re = %d",re);
    av_packet_unref(packet);
    mux.unlock();
    if(re < 0){

        XLOGT("avcodec_send_packet fail! this->isAudio : %d",this->isAudio);
        char buf[1024] = {0};
        av_strerror(re, buf, sizeof(buf)-1);
        XLOGT("%s", buf);
        return false;
...
XData FFDecode::RecvFrame()
{
    ...
    if(re < 0)
    {
        XLOGI("avcodec_receive_frame fail! re : %d",re);
        if (re == AVERROR_EOF){
            XLOGT("avcodec_send_packet AVERROR_EOF");
        }
        char buf[1024] = {0};
        av_strerror(re, buf, sizeof(buf)-1);
        XLOGE("%s", buf);
    ...
    if (re == AVERROR(EAGAIN)){
        XLOGT("avcodec_send_packet AVERROR(EAGAIN");
    ...
    if (frame)
    {
        if (!this->isAudio && this->isCapture) {
            AVFrame *pFrameRGB = av_frame_alloc();
            int PictureSize = avpicture_get_size (AV_PIX_FMT_BGR24, codec->width, codec->height);
            uint8_t *buf = (uint8_t*)av_malloc(PictureSize);
            if (buf != NULL)
            {
                avpicture_fill ( (AVPicture *)pFrameRGB, buf, AV_PIX_FMT_BGR24, codec->width, codec->height);
                //设置图像转换上下文
                SwsContext *pSwsCtx = sws_getContext (codec->width,
                                          codec->height,
                                          codec->pix_fmt,
                                          codec->width,
                                          codec->height,
                ...
                sws_scale (pSwsCtx, (const uint8_t **)frame->data, frame->linesize, 0, codec->height, pFrameRGB->data, pFrameRGB->linesize);
                SaveAsBMP(pFrameRGB->data[0],codec->width,codec->height,this->capturePath);
        ...
        //frame->pts = 0.5 * frame->pts;
        d.pts = frame->pts;
       // pts =  d.pts;
        //XLOGT("avcodec_receive_frame success 222, pts is  %lld  ", frame->pts);
    } else {
        XLOGD("frame null");
    }
    mux.unlock();
...
int FFDecode::initFilter(XParameter parameter){
    ...
    //3.1 获取输入filter
    AVFilter *buffersrc  = const_cast<AVFilter *>(avfilter_get_by_name("abuffer")); /* 输入buffer filter */
    ...
    //3.2 获取用于倍速播放的filter
    AVFilter *atempo_filter = const_cast<AVFilter *>(avfilter_get_by_name("atempo"));
    ...
    //3.3 获取输出filter
    AVFilter *buffersink = const_cast<AVFilter *>(avfilter_get_by_name("abuffersink")); /* 输出buffer filter */
...
int FFDecode::initFilter2(XParameter parameter){
    ...
    //3.1 获取filter
    AVFilter *buffersrc  = const_cast<AVFilter *>(avfilter_get_by_name("buffer")); /* 输入buffer filter */
    ...
    //3.2 获取filter
    AVFilter *buffersink = const_cast<AVFilter *>(avfilter_get_by_name("buffersink")); /* 输出buffer filter */
    ...
    filters_descr = filters_speed_video;
