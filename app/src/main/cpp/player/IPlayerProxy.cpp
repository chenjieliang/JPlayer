...
void IPlayerProxy::Init(void *vm)
{
    ...
    if(!player)
    {
        FFPlayerBuilder *playerBuilder = new FFPlayerBuilder();
        player = playerBuilder->BuilderPlayer();
    }
    mux.unlock();
}
...
void IPlayerProxy::InitNet(void *vm)
{
    ...
    if(!player)
    {
        ...
        FFPlayerBuilder *playerBuilder = new FFPlayerBuilder();
        player = playerBuilder->BuilderNetPlayer();
    }
    mux.unlock();
}
...
void IPlayerProxy::receiveVideoFrame(int type, int camType, int width, int height, int fps,
                                     int64_t pts, unsigned char *dataArray, long size)
{
    mux.lock();
    if(player)
    {
...
void IPlayerProxy::receiveAudioFrame(int type, int channels, int sps, int bps, int64_t pts,
...
void IPlayerProxy::capture(char *path, int type)
{
    mux.lock();
    if(player)
    {
        player->capture(path,type);
    }
    mux.unlock();
}

void IPlayerProxy::setSkipFrame(int skipNum) {
    mux.lock();
    if (player) {
        player->setSkipFrame(skipNum);
    }
    mux.unlock();
}